...
if ($tienda==='TODO' ) {
    ...
    if ($ncestado==='PENDIENTE' || $ncestado==='INSERTADO' || $ncestado==='EMITIDO' || $ncestado==='OBSERVADO' || 
    $ncestado==='INSTALADO' ) {


    $sqlncFija = " SELECT ncf.id_cargo,ncf.fecha_ingreso, ncf.ruc,ncf.razon_social,ncf.peticion,
 ncf.subtipo, ncf.descripcion , ncf.direccion, ncf.estado,s.nombre,ncf.ejecutivo_telefonica,ncf.back, usu.personal,ncf.nodo
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
...
ncf.estado in ('PENDIENTE','INSERTADO','EMITIDO','OBSERVADO','INSTALADO') and ncf.validacion='PROCEDE'  
...
SELECT ncf.id_cargo,ncf.fecha_ingreso, ncf.ruc,ncf.razon_social,ncf.peticion,
 ...
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
where month(ncf.fecha_liquidacion)='$periodo' and year(ncf.fecha_liquidacion)='$ano' and ncf.estado=
'LIQUIDADO' and ncf.validacion='PROCEDE'  
...
SELECT ncf.id_cargo,ncf.fecha_ingreso, ncf.ruc,ncf.razon_social,ncf.peticion,
 ...
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
where month(ncf.fecha_validacion)='$periodo' and year(ncf.fecha_validacion)='$ano' and ncf.estado=
'CAIDO' and ncf.validacion='PROCEDE'  
...
ORDER BY ncf.fecha_ingreso DESC";
 ...
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
where  year(ncf.fecha_validacion)<='$ano' and 
ncf.estado in ('PENDIENTE','INSERTADO','EMITIDO','OBSERVADO','INSTALADO') and ncf.validacion='PROCEDE'  and ncf.zonal='$tienda'
...
SELECT ncf.id_cargo,ncf.fecha_ingreso, ncf.ruc,ncf.razon_social,ncf.peticion,
 ...
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
where month(ncf.fecha_liquidacion)='$periodo' and year(ncf.fecha_liquidacion)='$ano' and ncf.estado=
'LIQUIDADO' and ncf.validacion='PROCEDE'  and ncf.zonal='$tienda'
...
SELECT ncf.id_cargo,ncf.fecha_ingreso, ncf.ruc,ncf.razon_social,ncf.peticion,
 ...
 from nc_avanzado as ncf left join supervisor as s on s.id_supervisor=ncf.id_supervisor left join usuario as usu on usu.id_usuario=ncf.id_usuario
where month(ncf.fecha_validacion)='$periodo' and year(ncf.fecha_validacion)='$ano' and ncf.estado=
'CAIDO' and ncf.validacion='PROCEDE'  and ncf.zonal='$tienda'
