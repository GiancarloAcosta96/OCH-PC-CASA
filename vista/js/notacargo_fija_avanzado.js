...
$(document).ready(function () {
  ...
  $('#suptipocom').click(function () {
    ...
    $('#btnRegistrar').click(function () {
            ...
            if( $("#suptipocom").val()== '0' || $("#descripcioncom").val()== '0' || 
                    $("#ncregion").val()== '0' || $("#ncback").val()== '0' || $("#nccargofijo").val()== '0' 
                    || $("#nccargofijo").val()== ''  || $("#ncnomcontacto").val()== '' 
                    || $("#nctelefono1").val()== '' || $("#ncdireccion").val()== '' 
                ...
                if (r!=1) {
                              ...
                              $('#modalEditar').modal('hide');
                               ...
                               var estado =$("#ncestado").val();
                                ...
                                $('#tablaDatatable').load('../capa_presentacion/tabla_nc_fija_avanzado.php?ano='+ano+'&periodo='+periodo+'&estado='+estado+'&idusu='+idusu);
         ...
         success: function (data) {
          ...
          if (datos.length > 0) {
                ...
                for (i = 0; i < datos.length; i++) {
                    $("#suptipocom").append("<option value = '" + datos[i][0] + "'>" + datos[i][0] + "</option>");
                }
            }
        }
    });
    ...
    $.ajax({
        ...
        url: "../procesos/obtenDatosDescripcionAv.php",
         success: function (data) {
            datos = jQuery.parseJSON(data);
          if (datos.length > 0) {
                $("#descripcioncom").append("<option value = '0'> --Elegir-- </option>");
                var i;
                for (i = 0; i < datos.length; i++) {
                    $("#descripcioncom").append("<option value = '" + datos[i][0] + "'>" + datos[i][0] + "</option>");
                }
            }
        }
    });
        ...
        url: "../procesos/obtenDatosback.php",
         success: function (r) {
          ...
          if (datos.length > 0) {
                ...
                var i;
                for (i = 0; i < datos.length; i++) {
                    $("#ncback").append("<option value = '" + datos[i][1] + "'>" + datos[i][1] + "</option>");
                }
            }
        }
    });
        ...
        url: "../procesos/obtenDatosUsuarioRegion.php",
         success: function (r) {
          ...
          if (datos.length > 0) {
                ...
                var i;
                for (i = 0; i < datos.length; i++) {
                    $("#ncregion").append("<option value = '" + datos[i][0] + "'>" + datos[i][1] + "</option>");
                }
            }
        }
    });
